...
<head>
	...
	<script type="text/javascript">
		 ...
		 */
		function add() {
			location.href = "adminType/toAddType";
		}
...
<div id="header">
	<ul class="nav nav-tabs">
		<li class="active"><a href="adminType/selectType">类型管理</a></li>
	</ul>
...
<div id="content">
	...
	<form id="form2" action="adminType/deleteType" name="deleteForm">
		<table class="table table-bordered table-hover">
			...
			<c:forEach items="${allTypes }" var="goodsType">
				<tr>
					<td><input type="checkbox" name="ids" value="${goodsType.id }"/></td>
					<td>${goodsType.id }</td>
					<td>${goodsType.typename }</td>
					<td><a href="adminType/selectAType?id=${goodsType.id }&act=updateAtype" target="center" >修改</a></td>
				</tr>
			</c:forEach>
			...
			<tr>
				<td colspan="8" align="right">
					...
					第${pageCur}页&nbsp;&nbsp;
					<c:url var="url_pre" value="adminType/selectType">
						<c:param name="pageCur" value="${pageCur - 1 }"/>
					</c:url>
					<c:url var="url_next" value="adminType/selectType">
						<c:param name="pageCur" value="${pageCur + 1 }"/>
					</c:url>
					<!-- 第一页没有上一页 -->
					<c:if test="${pageCur != 1 }">
	...
	<div id="btn-group">
		...
		<input type="button" value="删除" class="btn btn-danger btn-sm" onclick="confirmDelete1()"/>
	</div>
</div>
</body>
</html>